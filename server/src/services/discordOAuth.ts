...
export function signDiscordOAuthState(redirectUri: string): string {
  return jwt.sign(
...
export function verifyDiscordOAuthState(token: string): { redirect_uri: string; typ: 'discord_oauth' } {
  try {
...
export function assertRedirectUriAllowed(redirectUri: string): void {
  try {
    assertRedirectUri(redirectUri)
  } catch (err) {
    throw new TRPCError({ code: 'BAD_REQUEST', message: err instanceof Error ? err.message : 'redirect_uri is not allowed' })
  }
}

export async function exchangeDiscordCode(code: string, redirectUri: string): Promise<import('discord-oauth').DiscordTokenResponse> {
  try {
    ...
    throw new TRPCError({ code: code2, message })
  }
}

export async function fetchDiscordUserMe(discordAccessToken: string): Promise<import('discord-oauth').DiscordUser> {
  try {
