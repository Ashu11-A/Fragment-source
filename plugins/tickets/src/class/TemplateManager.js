...
import { ActionRowBuilder, ButtonInteraction, ButtonStyle, Colors, EmbedBuilder, Message, MessageComponentInteraction } from 'discord.js';
...
export class TemplateManager extends BaseInteractionBuilder {
    options;
    mode = 'production';
    type = TypeTemplate.Button;
    properties = {};
    selects = [];
    systems = [];
    switchToggles;
    ...
    renderedEmbed;
    renderedComponents;
    constructor({ interaction, template }) {
        super({ interaction });
        this.options = {
            title: undefined,
    ...
    setTitle(value) { this.options.title = value; return this; }
    setDescription(value) { this.options.description = value; return this; }
    setThumbnail(value) { this.options.thumbnail = value; return this; }
    setImage(value) { this.options.image = value; return this; }
    setColor(value) { this.options.color = value; return this; }
    setMode(value) { this.mode = value; return this; }
    setType(value) { this.type = value; return this; }
    setProperties(elements) { this.properties = elements ?? {}; return this; }
    setSystem(elements) { this.systems = elements ?? []; return this; }
    setSelects(selects) { this.selects = selects ?? []; return this; }
    switchData(data) { this.switchToggles = data; return this; }
    setData(data) {
    ...
    renderComponents() {
        ...
        const selects = [];
        if (this.mode === 'debug') {
            buttons.push(new ButtonBuilder({ customId: 'setTitle', style: ButtonStyle.Secondary, label: 'Nome', emoji: { name: '📝' } }), new ButtonBuilder({ customId: 'setDescription', style: ButtonStyle.Secondary, label: 'Descrição', emoji: { name: '📑' } }), new ButtonBuilder({ customId: 'setThumbnail', style: ButtonStyle.Secondary, label: 'Miniatura', emoji: { name: '🖼️' } }), new ButtonBuilder({ customId: 'setImage', style: ButtonStyle.Secondary, label: 'Banner', emoji: { name: '🌄' } }), new ButtonBuilder({ customId: 'setColor', style: ButtonStyle.Secondary, label: 'Cor', emoji: { name: '🎨' } }), new ButtonBuilder({ customId: 'SetSelect', style: ButtonStyle.Secondary, label: 'SelectMenu', emoji: { name: '🗄️' } }), new ButtonBuilder({ customId: 'AddSelect', style: ButtonStyle.Secondary, label: 'Add Select', emoji: { name: '📝' }, disabled: true }), new ButtonBuilder({ customId: 'SetButton', style: ButtonStyle.Secondary, label: 'Botão', emoji: { name: '🔘' } }), new ButtonBuilder({ customId: 'SetModal', style: ButtonStyle.Secondary, label: 'Modal', emoji: { name: '📄' } }), new ButtonBuilder({ customId: 'Category', label: 'Categoria', emoji: { name: '🔖' }, style: ButtonStyle.Secondary, disabled: true }), new ButtonBuilder({ customId: 'MoreDetails', label: 'Mais Detalhes', emoji: { name: '📃' }, style: ButtonStyle.Secondary }), new ButtonBuilder({ customId: 'Save', label: 'Salvar', emoji: { name: '✔️' }, style: ButtonStyle.Success }), new ButtonBuilder({ customId: 'DeleteTemplate', label: 'Apagar', emoji: { name: '✖️' }, style: ButtonStyle.Danger }));
        }
        else if (this.mode === 'production') {
            switch (this.type) {
                case TypeTemplate.Modal:
                case TypeTemplate.Button:
                    buttons.push(new ButtonBuilder({ customId: 'Open', label: 'Abrir Ticket', style: ButtonStyle.Success, emoji: { name: '🎫' } }), new ButtonBuilder({ customId: 'Config', emoji: { name: '⚙️' }, style: ButtonStyle.Secondary }));
                    break;
        ...
        for (const button of buttons) {
            ...
            if (this.systems.find((mod) => mod.name === button.customId && mod.isEnabled))
                button.setStyle(ButtonStyle.Primary);
            if (mappedType !== undefined && this.type === mappedType)
                button.setStyle(ButtonStyle.Primary);
            if (customId === 'AddSelect' && this.type === TypeTemplate.Select)
                button.setDisabled(false);
            if (customId === 'Category' && (this.type === TypeTemplate.Button || this.type === TypeTemplate.Modal))
                button.setDisabled(false);
            if (customId === 'MoreDetails' && this.type === TypeTemplate.Modal)
                button.setDisabled(true);
            if (this.properties[customId] === true)
                button.setStyle(ButtonStyle.Primary);
        }
    ...
    get embeds() {
        if (this.renderedEmbed === undefined)
            this.renderEmbed();
        return this.renderedEmbed !== undefined ? [this.renderedEmbed] : undefined;
    }
    get components() {
        if (this.renderedComponents === undefined)
    ...
    async delete({ messageId }) {
        ...
        await database.template.delete({ messageId }).then(async (result) => {
            ...
            if ((result?.affected ?? 0) > 0) {
                ...
                if (isCollector) {
                    await this.interaction.update({ embeds: [embed], components: [] });
                    return;
                }
