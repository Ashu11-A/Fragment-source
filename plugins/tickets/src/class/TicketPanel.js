...
import { ActionRowBuilder, ButtonBuilder, ButtonInteraction, ButtonStyle, ChannelType, CommandInteraction, EmbedBuilder, ModalSubmitInteraction, StringSelectMenuInteraction, TextInputBuilder, TextInputStyle } from 'discord.js';
...
export class TicketPanel {
    ...
    async CreateCall() {
        ...
        if (!this.interaction.inCachedGuild())
            return;
        if (await this.validator() || channelId === null)
            return;
        const ticketData = await database.ticket.findOne({ where: { channelId } });
        if (ticketData === null)
            return await new DiscordError({ element: 'ticket', interaction: this.interaction }).reply();
    ...
    async AddUser() {
        ...
        const input = new ActionRowBuilder().addComponents(new TextInputBuilder({
            customId: 'id',
    ...
    async RemoveUser() {
        ...
        for (const user of (ticketData?.users ?? [])) {
            if (typeof options.find((option) => option.value === user.id) === 'object')
                continue;
            options.push({
