...
function formatTranscriptText(displayName, fields, history) {
...
export class Ticket {
    ...
    async transcript(options) {
        ...
        const channel = config?.logsId != null
            ...
            : await createChannel();
        if (config !== null && config.logsId == null)
            await database.config.save(Object.assign(config, { logsId: channel.id }));
        ...
        if (typeof observation === 'string')
            embed.addFields({ name: '📝 Observação', value: codeBlock(observation) });
        if (typeof reason === 'string')
            embed.addFields({ name: '👉 Motivo do atendimento', value: codeBlock(reason) });
        const text = formatTranscriptText(user?.displayName ?? ticket.ownerId, embed.data.fields ?? [], ticket.history ?? []);
        if (!channel.isTextBased())
            return await new DiscordError({ element: `pois o channel: ${channel.name}, não tem a permissão de envio de mensagens.`, interaction: this.interaction }).notPossible().reply();
