...
import { ActionRowBuilder, ButtonStyle, ChannelType, codeBlock, CommandInteraction, EmbedBuilder, Message, type OverwriteResolvable, PartialGroupDMChannel, PermissionsBitField, TextChannel, User } from 'discord.js'
...
export class TicketBuilder {
  ...
  async create (): Promise<Ticket | null | undefined | void> {
    ...
    if (templateData === null) return await new DiscordError({ element: 'esse template', interaction: this.interaction }).notFound({ type: 'Database' }).reply()
    ...
    if (guildRelaction === null) return await new DiscordError({ element: 'Guild', interaction: this.interaction }).notFound({ type: 'Database' }).reply()

    const templateRelaction = await database.template.findOne({ where: { id: this.templateId } })
    if (templateRelaction === null) return await new DiscordError({ element: 'Template', interaction: this.interaction }).notFound({ type: 'Database' }).reply()
  ...
  async loader() {
    if (this.channelId === undefined) throw new DiscordError({ element: 'executar essa ação, pois setTicket não foi configurado!', interaction: this.interaction }).notPossible().reply()
  ...
  async delete (options?: { reason?: string, observation?: string }) {
    ...
    if (ticketData === null) { await new DiscordError({ element: 'as informações do ticket', interaction: this.interaction }).notFound({ type: 'Database' }).reply(); return }
    
    const channel = await this.interaction.client.channels.fetch(ticketData.channelId).catch(() => null)
    if (!channel?.isTextBased()) { await new DiscordError({ element: ticketData.channelId, interaction: this.interaction }).notFound({ type: 'Channel' }).reply(); return }
  ...
  async edit () { 
    if (this.ticketId === undefined) throw new DiscordError({ element: 'executar essa ação, pois o Id do ticket não foi setado!', interaction: this.interaction }).notPossible().reply()
  ...
  async send (embeds: EmbedBuilder[]) {
    ...
    const channel = await this.interaction.client.channels.fetch(ticketData.channelId).catch(() => null)
    if (!channel?.isTextBased() || channel instanceof PartialGroupDMChannel) { await new DiscordError({ element: ticketData.channelId, interaction: this.interaction }).notFound({ type: 'Channel' }).reply(); return }
  ...
  async update () {
    ...
    if (message === null) { await new DiscordError({ element: ticketData.messageId, interaction: this.interaction }).notFound({ type: 'Message' }).reply(); return }
