...
import { ActionRowBuilder, ButtonBuilder as DjsButtonBuilder, ButtonInteraction, ButtonStyle, ChannelType, codeBlock, CommandInteraction, EmbedBuilder, Message, ModalSubmitInteraction, PermissionsBitField, StringSelectMenuInteraction } from 'discord.js';
export class ClaimBuilder {
    ...
    setTicketId(ticketId) { this.options.ticketId = ticketId; return this; }
    setData(ticket) { this.ticketData = ticket; return this; }
    permissions(roles) {
    ...
    async render() {
        ...
        if (ticketData === null || ticketData === undefined)
            throw await new DiscordError({ element: 'ticket', interaction: this.interaction }).notFound({ type: 'Database' }).reply();
    ...
    async create() {
        ...
        const ticketData = this.ticketData !== undefined ? this.ticketData : await database.ticket.findOne({ where: { id: this.options.ticketId } });
        if (ticketData === null)
            throw await new DiscordError({ element: 'executar a ação de criação do claim, pois o setData ou setTicketId não foi definido!', interaction: this.interaction }).notPossible().reply();
        const configs = await database.config.findOne({ where: { guild: { guildId: guildId } }, relations: { guild: true } });
        ...
        async function createChannel() {
            ...
            await database.config.update({ id: configs.id }, { claimId: newGuild.id });
            return newGuild;
        }
        ...
        if (channel.isTextBased()) {
            const message = await channel.send({ embeds: [this.embed], components: this.buttons });
            const claimData = await database.claim.create({ channelId: channel.id, messageId: message.id, ticket: ticketData ?? undefined });
            const result = await database.claim.save(claimData);
    ...
    async edit({ messageId }) {
        ...
        if (claimData === null)
            throw await new DiscordError({ element: 'o claim', interaction: this.interaction }).notFound({ type: 'Database' }).reply();
        const channel = await this.interaction.client.channels.fetch(claimData.channelId).catch(async () => null);
        if (channel === null)
            throw await new DiscordError({ element: 'do claim', interaction: this.interaction }).notFound({ type: 'Channel' }).reply();
        if (!channel.isTextBased())
            throw await new DiscordError({ element: 'concluir a ação, pois o channel não é um TextBased', interaction: this.interaction }).notPossible().reply();
        if (this.embed === undefined || !this.buttons)
            await this.render();
        const message = await channel.messages.fetch(claimData.messageId).catch(() => null);
        if (message === null)
            throw await new DiscordError({ element: 'message do claim', interaction: this.interaction }).notFound({ type: 'Database' }).reply();
        await message.edit({
            embeds: [this.embed],
    ...
    async delete(id) {
        ...
        if (claimData === null)
            return await new DiscordError({ element: 'claim', interaction: this.interaction }).notFound({ type: 'Database' }).reply();
        const channel = await this.interaction.client.channels.fetch(claimData.channelId).catch(async () => null);
        if (channel === null)
            return await new DiscordError({ element: 'do claim', interaction: this.interaction }).notFound({ type: 'Channel' }).reply();
        if (!channel.isTextBased())
            return await new DiscordError({ element: 'concluir a ação, pois o channel não é um TextBased', interaction: this.interaction }).notPossible().reply();
        const message = await channel.messages.fetch(claimData.messageId).catch(() => null);
        if (message !== null && message.deletable)
            await message.delete();
        await database.claim.delete({ id });
        return this;
    }
}
