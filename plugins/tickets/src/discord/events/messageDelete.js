...
import { AuditLogEvent, EmbedBuilder, Guild, Message, MessageFlagsBitField } from 'discord.js';
async function sendDeletionWarning(channel, guild) {
...
export default new Event({
    ...
    async run(message) {
        if (!message.inGuild())
            return;
        if (message.flags.has(MessageFlagsBitField.Flags.Ephemeral))
            return;
        const { id } = message;
        ...
        if (claimData !== null && claimData.ticket?.id !== undefined) {
            ...
            const newMessage = await message.channel.send({ embeds: [builder.embed], components: builder.buttons });
            await database.claim.save(Object.assign(claimData, { messageId: newMessage.id }));
            await sendDeletionWarning(message.channel, message.guild);
            return;
        }
