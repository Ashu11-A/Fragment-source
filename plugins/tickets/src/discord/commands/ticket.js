...
import { AddSelectButton, AddSelectModal, Category, Config, DeleteTemplate, EditSelectMenu, ModalOpen, MoreDetailsModal, MoreDetailsToggle, Open, Save, SelectCategory, SelectMenu, SetButton, SetColorButton, SetColorModal, SetDescriptionButton, SetDescriptionModal, SetImageButton, SetImageModal, SetModal, SetSelect, SetThumbnailButton, SetThumbnailModal, SetTitleButton, SetTitleModal, } from '../components/TemplateActions.js';
import { Command, DiscordError } from 'discord';
import { ApplicationCommandOptionType, ApplicationCommandType, Colors, EmbedBuilder, MessageFlags, PermissionFlagsBits } from 'discord.js';
            ...
            switch (options.getSubcommandGroup()) {
                case 'category': {
                    switch (options.getFocused(true).name) {
                        ...
                        case 'category': {
                            const messageId = options.data[0].options?.[0].options?.find((option) => option.name === 'message_id')?.value;
            ...
            switch (options.getSubcommand()) {
                case 'template': {
                    if (!interaction.inCachedGuild())
                        throw await new DiscordError({ element: 'a ação não foi realizada dentro de um servidor', interaction }).notPossible().reply();
                    const sendChannel = await interaction.guild.channels.fetch(channelId);
                    if (!sendChannel?.isTextBased())
                        throw await new DiscordError({ element: 'concluir a ação, pois o channel não é um TextBased', interaction }).notPossible().reply();
                ...
                case 'change': {
                    ...
                    if (isDelete) {
                        await builder.delete({ messageId: templateId });
                        return;
                    }
                    if (title !== null)
