...
export const Claim = new Button({
  ...
  async onClick(interaction) {
    ...
    if (claimData === undefined || claimData?.ticket === undefined) return await new DiscordError({ element: 'Claim', interaction }).notFound({ type: 'Database' }).reply()
...
export const Delete = new Button({
  ...
  async onClick(interaction) {
    ...
    const claimData = await database.claim.findOne({ where: { messageId: message.id }, relations: { ticket: true } })
    if (claimData === null) throw await new DiscordError({ element: 'este claim', interaction }).notFound({ type: 'Database' }).reply()
