...
import { ActionRowBuilder, ButtonBuilder, ButtonStyle, ComponentType, EmbedBuilder, MessageFlags, PermissionsBitField, StringSelectMenuBuilder, TextInputBuilder, TextInputStyle, } from 'discord.js';
...
export const EditSelectMenu = new StringSelect({
    ...
    async onSelect(interaction) {
        ...
        if (!interaction.memberPermissions.has(PermissionsBitField.Flags.Administrator))
            throw await new DiscordError({ element: 'você', interaction }).forbidden().reply();
        ...
        if (templateData === null)
            throw await new DiscordError({ element: 'template', interaction }).notFound({ type: 'Database' }).reply();
...
export const AddSelectButton = new Button({
    ...
    async onClick(interaction) {
        ...
        for (const element of elementsSelect) {
            modal.addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder(element)));
        }
...
// ─── Set Type Buttons ─────────────────────────────────────────────────────────
async function runSetType(interaction, type) {
    await interaction.deferReply({ flags: MessageFlags.Ephemeral });
    const templateData = await database.template.findOne({ where: { messageId: interaction.message.id } });
    if (templateData !== null) {
        ...
        if ((templateData.categories ?? []).length > 0) {
            ...
            switch (templateData.type) {
                ...
                case TypeTemplate.Modal: {
                    ...
                    const select = ActionDrawer([new StringSelectMenuBuilder({ customId: 'SelectCategory', placeholder: 'Selecione a categoria do seu Ticket', minValues: 1, maxValues: 1, options })], 1);
                    await interaction.editReply({ components: select });
                }
            }
        }
        else {
            if (templateData.type === TypeTemplate.Button && !moreDetails)
                await interaction.deferReply({ flags: MessageFlags.Ephemeral });
            switch (templateData.type) {
                case TypeTemplate.Button: {
                    if (moreDetails) {
                        const modal = new ModalBuilder({ customId: 'MoreDetails', title: 'Conte-nos mais sobre o seu problema' });
                        const rows = ActionDrawer([
...
export const MoreDetailsModal = new Modal({
    ...
    async onSubmit(interaction) {
        ...
        if (cache !== undefined)
            cacheSelectMenu.delete(user.id);
        const builder = new TicketBuilder({ interaction }).setOwner(user.id).setDescription(description);
        if (cache !== undefined)
            builder.setTitle(cache.title).setCategory(cache);
        await builder.create();
    },
});
export const SelectCategory = new StringSelect({
    ...
    async onSelect(interaction) {
        ...
        const [messageId, index] = values[0].split('_');
        ...
        if (category === undefined)
            throw await new DiscordError({ element: 'categoria', interaction }).notFound({ type: 'Database' }).reply();
        switch (templateData.type) {
            case TypeTemplate.Button:
...
export const SelectMenu = new StringSelect({
    ...
    async onSelect(interaction) {
        ...
        if (typeTicket === undefined)
            throw await new DiscordError({ element: 'select', interaction }).notFound({ type: 'Database' }).reply();
        const moreDetails = (templateData.systems ?? []).find((system) => system.name === 'MoreDetails')?.isEnabled;
        if (moreDetails) {
            const modal = new ModalBuilder({ customId: 'MoreDetails', title: 'Conte-nos mais sobre o seu problema' });
            const row = ActionDrawer([new TextInputBuilder({ customId: 'description', label: 'Qual a descrição?', required: true, maxLength: 255, style: TextInputStyle.Paragraph, placeholder: 'Queria saber mais informações sobre...' })], 1);
