...
import { ActionRowBuilder, ButtonBuilder, ButtonStyle, ComponentType, EmbedBuilder, MessageFlags, StringSelectMenuBuilder, TextInputBuilder, TextInputStyle, } from 'discord.js';
...
export const Switch = new Button({
    ...
    async onClick(interaction) {
        ...
        if (ticketData === null)
            return await new DiscordError({ element: `o ticket ${channelId}`, interaction }).notFound({ type: 'Database' }).reply();
        ...
        const ticket = await (await new TicketBuilder({ interaction })
            ...
            .update())?.send([embed]);
        await new ClaimBuilder({ interaction }).setData(ticket?.options).edit({ messageId: claimId });
        if (ephemeral) {
...
export const CloseWithQuestion = new Button({
    ...
    async onClick(interaction) {
        ...
        modal.setComponents(ActionDrawer([
...
export const PanelSelect = new StringSelect({
    ...
    async onSelect(interaction) {
        ...
        switch (values[0]) {
            case 'CreateCall':
                await panel.CreateCall();
                break;
            case 'AddUser':
                await panel.AddUser();
                break;
            case 'RemoveUser':
                await panel.RemoveUser();
                break;
            case 'Transcript':
                await ticket.transcript({ channelId });
                break;
            case 'Delete':
                await ticket.delete({ channelId });
                break;
        }
    },
});
