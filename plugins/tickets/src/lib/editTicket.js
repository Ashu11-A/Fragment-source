...
import { ActionRowBuilder, ComponentType, EmbedBuilder, MessageFlags, TextInputBuilder, } from 'discord.js';
...
export async function runEditButton(interaction, action) {
    const data = modalData[action];
    if (data === undefined)
        return;
    const templateData = await database.template.findOne({ where: { messageId: interaction.message.id } });
    const value = templateData?.embed?.[data.database];
    ...
    const modal = new ModalBuilder({
        ...
        components: [new ActionRowBuilder().addComponents(new TextInputBuilder({ ...data, value: typeof value !== 'string' ? undefined : value }))]
    });
    await interaction.showModal(modal);
}
export async function runEditModal(interaction, action) {
