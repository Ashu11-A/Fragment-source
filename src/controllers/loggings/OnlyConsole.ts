...
import { type Colors } from '@/controllers/loggings/Types'
...
export function Message (args: LogMessage[]): void {
  const message = WhiteColors(args)

  // Atualize a função MakeLog para aplicar cores na mensagem
  function MakeLog (message: string): void {
    console.log(`${message}`)
  }

  MakeLog(message)
}
export function Console (controller: string, color: string, Type: string, args: LogMessage[]): void {
  const message = WhiteColors(args)
